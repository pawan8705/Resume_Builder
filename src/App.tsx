// src/App.tsx — Complete app with all routes
...
function Loader() {
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center ${isDark?'bg-[#08080f]':'bg-[#f1f5f9]'}`}>
      <div className="flex flex-col items-center gap-3">
        <div className="size-12 rounded-2xl bg-gradient-to-br from-violet-600 to-blue-600 flex items-center justify-center text-2xl animate-pulse shadow-[0_0_24px_rgba(124,58,237,0.5)]">
          📄
        </div>
...
function ThemeSync({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      ::-webkit-scrollbar-thumb { background: ${theme==='dark'?'rgba(255,255,255,0.12)':'rgba(0,0,0,0.12)'}; border-radius: 999px; }
      ::-webkit-scrollbar-thumb:hover { background: ${theme==='dark'?'rgba(255,255,255,0.2)':'rgba(0,0,0,0.2)'}; }
    `
    if (!document.getElementById('scrollbar-style')) document.head.appendChild(style)
  }, [theme])
  return <>{children}</>
}
