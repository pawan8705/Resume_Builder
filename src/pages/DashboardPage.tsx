...
const QUICK_ACTIONS = [
  {
    ...
    label: "Create Resume",
    sub: "Start fresh",
    grad: "from-violet-600 to-blue-600",
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Your resume data is saved locally.
        </p>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3
          className={`text-base font-black mb-1.5 ${isDark ? "text-slate-100" : "text-slate-900"}`}
        >
          Delete Resume?
        </h3>
        <p
    ...
    >
                ...
                {[
                  {
                    ...
                    label: "Edit",
                    fn: onEdit,
                    clr: isDark ? "text-slate-200" : "text-slate-700",
                  },
                  {
                    icon: Copy,
                    label: "Duplicate",
      ...
      <div className="mb-3.5">
        <div className="flex justify-between mb-1.5">
          ...
          <span className={`text-[11px] font-black ${atsColor}`}>
            {resume.atsScore}%
          </span>
        </div>
        <div
      ...
      <div
        className={`flex items-center gap-3 mb-3.5 text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}
      >
        <span className="flex items-center gap-1">
          <Eye size={11} /> {resume.views}
        </span>
        <span className="flex items-center gap-1">
          <Download size={11} /> {resume.downloads}
        </span>
        <span className="ml-auto flex items-center gap-1">
          <Clock size={10} /> {resume.lastEdited}
        </span>
      </div>
...
export default function DashboardPage() {
  ...
  const avgATS = resumes.length
    ? Math.round(resumes.reduce((a, b) => a + b.atsScore, 0) / resumes.length)
    : 0;
  ...
  const SidebarContent = () => (
    <div className={`flex flex-col h-full ${side}`}>
      <div
        className={`px-4 pt-4 pb-3.5 border-b ${divB} flex items-center justify-between`}
      >
        <Link to="/" className="flex items-center gap-2 no-underline">
          <div
            className="size-8 rounded-xl bg-gradient-to-br from-violet-600 to-blue-600 flex items-center justify-center text-sm shrink-0"
        ...
        <button
          ...
          className={`md:hidden size-8 rounded-xl border-none cursor-pointer flex items-center justify-center ${isDark ? "bg-white/6 text-slate-400" : "bg-black/5 text-slate-500"}`}
        >
          <X size={15} />
        </button>
      </div>
      <div className={`px-3 py-2 border-b ${divB}`}>
        <Link
          to="/"
          className={`flex items-center gap-2 px-2 py-2 rounded-lg text-xs font-medium no-underline transition-colors ${isDark ? "text-slate-500 hover:text-slate-200 hover:bg-white/5" : "text-slate-400 hover:text-slate-700 hover:bg-black/4"}`}
        >
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border-l-2 text-sm font-medium cursor-pointer border-none transition-all
                ${
                  active
                    ? isDark
                      ? "bg-violet-500/12 border-violet-500 text-violet-400 font-semibold"
      ...
      <div className={`p-3 border-t ${divB}`}>
        <div
          className={`flex items-center gap-2.5 p-3 rounded-xl mb-2.5 ${isDark ? "bg-white/3" : "bg-black/3"}`}
        >
          <div className="size-9 rounded-full bg-gradient-to-br from-violet-600 to-blue-600 flex items-center justify-center text-white text-xs font-black shrink-0">
            {initials}
          </div>
          <div className="min-w-0 flex-1">
            <p
              className={`text-xs font-bold truncate ${isDark ? "text-slate-100" : "text-slate-900"}`}
            >
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex-1 py-2 rounded-xl text-xs font-medium cursor-pointer border bg-transparent ${isDark ? "border-white/10 text-slate-400" : "border-black/10 text-slate-500"}`}
          >
            {isDark ? "☀️ Light" : "🌙 Dark"}
          </button>
          <button
            onClick={() => setShowLogout(true)}
            className="flex-1 py-2 rounded-xl text-xs font-semibold cursor-pointer border-none bg-red-500/10 text-red-400 flex items-center justify-center gap-1"
          >
  ...
  return (
    <>
      ...
      <div className={`flex h-screen overflow-hidden ${bg}`}>
        {/* Desktop Sidebar */}
        <div
          className={`hidden md:flex flex-col w-[220px] shrink-0 border-r ${divB}`}
        >
          <SidebarContent />
        </div>
        ...
        <div className={`flex-1 flex flex-col overflow-hidden min-w-0 ${main}`}>
          {/* Topbar */}
          <div
            className={`h-14 px-4 flex items-center gap-3 shrink-0 border-b backdrop-blur-xl sticky top-0 z-20 ${isDark ? "bg-[#0d0d1e]/95 border-white/[0.07]" : "bg-white/95 border-black/[0.07]"}`}
          >
            <button
              onClick={() => setMobile(true)}
              className={`md:hidden size-9 rounded-xl border-none cursor-pointer flex items-center justify-center shrink-0 ${isDark ? "bg-white/6 text-slate-200" : "bg-black/5 text-slate-700"}`}
            >
              <Menu size={17} />
            </button>
            <div className="flex-1 min-w-0">
              <h1
                className={`text-lg font-black leading-none tracking-tight ${isDark ? "text-slate-100" : "text-slate-900"}`}
              >
                Dashboard
              </h1>
              <p
            ...
            {/* Search */}
            <div
              className={`hidden sm:flex items-center gap-2 px-3.5 py-2 rounded-xl border ${isDark ? "bg-white/5 border-white/8" : "bg-black/4 border-black/8"}`}
            >
              <Search
                size={13}
            ...
            <button
              className={`relative size-9 rounded-xl border-none cursor-pointer flex items-center justify-center shrink-0 ${isDark ? "bg-white/5 text-slate-400" : "bg-black/4 text-slate-500"}`}
            >
              <Bell size={15} />
              <span
                className="absolute top-1.5 right-1.5 size-2 rounded-full bg-violet-500"
                style={{ border: `2px solid ${isDark ? "#0d0d1e" : "#fff"}` }}
              />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden">
            <div className="p-4 sm:p-5 md:p-6 max-w-6xl mx-auto">
                ...
                {STATS.map((stat, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <span
                        className={`text-[11px] font-semibold leading-tight ${isDark ? "text-slate-500" : "text-slate-400"}`}
                      >
                        {stat.label}
                      </span>
                      <div
                        className={`size-8 rounded-xl flex items-center justify-center shrink-0 ${stat.iconBg}`}
                      >
                        <stat.icon size={15} className={stat.color} />
                      </div>
                    </div>
                    <p
                      className={`text-2xl font-black tracking-tight leading-none ${isDark ? "text-slate-100" : "text-slate-900"}`}
                    >
              ...
              <div className="mb-5">
                <h2
                  className={`text-sm font-bold mb-3 ${isDark ? "text-slate-300" : "text-slate-700"}`}
                >
                  Quick Actions
                </h2>
                    ...
                    >
                      {action.link ? (
                        <Link to={action.link} className="no-underline block">
                          <div
                            className={`flex items-center gap-3 p-3.5 rounded-2xl border cursor-pointer transition-all ${isDark ? "bg-white/[0.03] border-white/[0.07] hover:bg-white/[0.055]" : "bg-white border-black/[0.07] shadow-sm hover:shadow-md"}`}
                          >
                            <div
                              className={`size-10 rounded-xl bg-gradient-to-br ${action.grad} flex items-center justify-center shrink-0`}
                              style={{
                        ...
                        >
                          <div
                            className={`size-10 rounded-xl bg-gradient-to-br ${action.grad} flex items-center justify-center shrink-0`}
                            style={{ boxShadow: `0 4px 12px ${action.shadow}` }}
                          >
                            <action.icon size={17} className="text-white" />
                          </div>
                          <div className="min-w-0">
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  >
                    My Resumes{" "}
                    <span
                      className={`text-xs font-normal ml-1 ${isDark ? "text-slate-600" : "text-slate-400"}`}
                    >
                ...
                {/* Mobile search */}
                <div
                  className={`sm:hidden flex items-center gap-2 px-3.5 py-2.5 rounded-xl border mb-3 ${isDark ? "bg-white/5 border-white/8" : "bg-white border-black/8"}`}
                >
                  <Search
                    size={13}
                    className={isDark ? "text-slate-500" : "text-slate-400"}
                ...
                {filteredResumes.length === 0 ? (
                  <div
                    className={`text-center py-12 rounded-2xl border ${isDark ? "bg-white/[0.02] border-white/[0.06]" : "bg-white border-black/[0.06]"}`}
                  >
                    <FileText
                      size={32}
                    ...
                    />
                    <p
                      className={`text-sm font-semibold mb-1 ${isDark ? "text-slate-500" : "text-slate-400"}`}
                    >
                      No resumes found
                    </p>
                    <p
                      className={`text-xs mb-4 ${isDark ? "text-slate-600" : "text-slate-400"}`}
                    >
                      {search
